...
所有新增建议和修正都请通过 GitHub Issue 提出：

- **推荐新数据集**：提交 Issue
- **提出资料修正**：如果元数据不准确、链接失效、限制条件缺失或分类值得重新讨论，请提交 Issue
- **报告数据更新**：请在 Issue 中附上相关官方来源

推荐数据集时，请尽量提供官方链接、访问和许可信息、一级任务，以及中英文简介。

权威记录位于 [`data/datasets/`](./data/datasets/)，每个数据集对应一个 JSON 文件。[`datasets.json`](./datasets.json) 和两个 README 中的数据目录均由这些记录生成。

维护者使用 `npm run check` 重新生成并校验目录。可通过 `npm run check:links` 审计外部链接。明确的 HTTP 错误会导致检查失败；网络超时只会标记为待验证，不会自动删除资源。
