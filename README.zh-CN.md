...
普通静态分类数据集、只有模型代码的仓库，以及没有明确数据访问路径的资源不在收录范围内。“开放”不一定意味着没有限制：部分数据集需要注册、申请或接受非商业条款。使用前应始终核对官方许可。

## 贡献与修正

欢迎通过社区方式参与维护：

- **推荐新数据集**：提交 Issue 或 Pull Request
- **提出资料修正**：如果元数据不准确、链接失效、限制条件缺失或分类值得重新讨论，请提交 Issue
- **提交数据更新**：有官方来源支持时，可通过 Pull Request 修改记录

推荐数据集时，请确认它符合收录标准，检查是否与已有数据集或前代版本重复，只选择一个一级任务，核实官方链接，说明访问和许可限制，并补充中英文字段。

权威记录位于 [`data/datasets/`](./data/datasets/)，每个数据集对应一个 JSON 文件。[`datasets.json`](./datasets.json) 和两个 README 中的数据目录均由这些记录生成。

提交 Pull Request 前，可使用以下命令重新生成并校验目录：

```bash
npm run check
```

可通过 `npm run check:links` 审计外部链接。明确的 HTTP 错误会导致检查失败；网络超时只会标记为待验证，不会自动删除资源。
